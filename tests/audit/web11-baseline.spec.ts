  ...
  test(`WEB11-00 baseline ${viewport.width}`, async ({ page }) => {
    ...
    const capture = async (state: string, selector?: string) => {
      ...
      const layout = await page.evaluate(() => ({
        ...
        todayRows: Array.from(document.querySelectorAll(".today-row"), (node) => {
          const rect = node.getBoundingClientRect();
          return { x: rect.x, y: rect.y, width: rect.width, height: rect.height };
        }),
        manualChecks: document.querySelectorAll('.today-row input[type="checkbox"]').length,
    ...
    const completedBeforeTimer = await page.locator('.today-row input:checked').count();
    ...
    await expect(page.getByRole("dialog", { name: "おつかれさまでした" })).toBeVisible();
    await capture("completion-dialog");
    await page.getByRole("button", { name: "今は変更しない" }).click();
    await expect(page.locator('.today-row input:checked')).toHaveCount(completedBeforeTimer);
    await capture("after-timer", ".today-section");
    for (const checkbox of await page.locator('.today-row input[type="checkbox"]').all()) {
    ...
    await writeFile(path.join(output, `${viewport.width}-metrics.json`), JSON.stringify(metrics, null, 2) + "\n");
  });
}
